...
sample_t genSampPxy(int nsamp,const Mat& pxy){
	Vec px = pxy.rowwise().sum();
	Vec py = pxy.colwise().sum();
	Mat pxcy = pxy * (1.0/py.array()).matrix().asDiagonal();
	Vec cumsum_py = Vec(pxy.cols());
	double pyacc = 0;
	for(size_t nn=0;nn<pxy.cols();++nn){
		pyacc += py[nn];
		cumsum_py[nn] = pyacc;
	}
	Mat cumsum_pxcy = Mat(pxy.rows(),pxy.cols());
	for(size_t nc = 0;nc<pxy.cols();++nc){
		double tmp_acc = 0;
		for(size_t nr =0; nr<pxy.rows();++nr){
			tmp_acc += pxcy(nr,nc);
			cumsum_pxcy(nr,nc) = tmp_acc;
		}
	}
	// gen y samples
	...
	// counting sample pxy
	Mat tmp_pxy = Mat(pxy.rows(),pxy.cols());
	for(size_t tt=0;tt<nsamp;tt++){
		tmp_pxy(static_cast<int>(x_samp[tt]),static_cast<int>(y_samp[tt]))+=1.0;
	}
	tmp_pxy *= (1.0/(double)nsamp);
	sample_t output = {.samp_pxy=tmp_pxy, .x_samp=x_samp, .y_samp=y_samp};
	return output;
}
